<html>
<head> <title>Library</title> <style>
ul {
...
die('Could not connect: ' . mysql_error());} $sql = "SELECT count(id) FROM library "; $retval = mysqli_query( $link, $sql );
if(! $retval ) {
...
"FROM library ". "WHERE id='$id'" . "LIMIT $offset, $max";
$retval = mysqli_query($link,$sql ); if(! $retval ) {
die('Could not get data: ' . mysql_error());}
while($row = mysqli_fetch_array($retval, MYSQL_ASSOC)) {
echo "Book ID :{$row['bid']} <br> ". "Book Name : {$row['bname']} <br> ". "Fine Paid : {$row['finepaid']} <br> ". “--------------------------------<br>";}
mysqli_close($link); ?>
</body> 
</html>